...
def process_liquidation(data):
    ...
    try:
        # Check if it's a liquidation event (adapt based on Paradex structure)
        # Assuming Paradex uses similar structure to trades for liquidations
        if data.get('method') == 'subscription' and data.get('params', {}).get('channel') == 'trades':
            trade_data = data.get('params', {}).get('data', {})
            
            timestamp = pd.Timestamp.utcnow()
            
            # Create unique ID
            hash_input = f"{timestamp}:{trade_data.get('market')}:{trade_data.get('side')}:{trade_data.get('size')}".encode('utf-8')
...
async def paradex_websocket():
    """Connect to Paradex WebSocket and process liquidations"""
    # Use JSON-RPC format like in paradex_monitor.py
    subscribe_msg = {
        "jsonrpc": "2.0",
        "method": "subscribe",
        "params": {"channel": "trades.ALL"},  # Assuming trades channel for liquidations
        "id": 1
    }
    while True:
        try:
            async with websockets.connect(PARADEX_WS_URL) as websocket:
                print("Connected to Paradex WebSocket")
                # Send subscription message
                await websocket.send(json.dumps(subscribe_msg))
                print("Subscription message sent for trades.ALL")
                while True:
                    try:
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("### Powered by [Paradex.trade](https://paradex.trade)")
        st.markdown("""
        <div style="background-color: #1E1E1E; padding: 15px; border-radius: 10px; margin-top: 20px;">
            <p style="margin-bottom: 10px;">💸 Start trading on Paradex with my referral link:</p>
            <p style="margin-bottom: 10px; font-size: 14px; color: #888;">
                <strong>You receive 5% back</strong> from all fees
            </p>
            <a href="https://paradex.trade" target="_blank" style="color: #4CAF50; text-decoration: none;">
                https://paradex.trade
            </a>
        </div>
        ...
        <div style="text-align: center; margin-top: 10px;">
            <a href="https://github.com/cj4c0b1/paradex-lik-dash" target="_blank" style="text-decoration: none; color: #ffffff; font-weight: bold;">
